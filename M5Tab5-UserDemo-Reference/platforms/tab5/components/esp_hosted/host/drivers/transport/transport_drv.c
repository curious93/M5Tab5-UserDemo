...
static esp_err_t transport_drv_sta_tx(void *h, void *buffer, size_t len)
{
	...
	if (!copy_buff) {
		/* DMA mempool exhausted under sustained CDN streaming.
		 * Drop this TX frame; lwip will retransmit. Asserting here
		 * caused full reboot loops on M5Tab5/P4 with cspot. */
		errno = -ENOBUFS;
#if defined(ESP_ERR_ESP_NETIF_TX_FAILED)
		return ESP_ERR_ESP_NETIF_TX_FAILED;
#else
