...
#error please disable LV_COLOR_DEPTH_1 for using monochromatic screen
    ...
    if (disp_cfg->flags.sw_rotate) {
        /* PPA requires cache-line aligned buffers */
        size_t rot_buf_size = ALIGN_UP_BY(buffer_size * color_bytes, data_cache_line_size);
        disp_ctx->draw_buffs[2] = heap_caps_aligned_alloc(data_cache_line_size, rot_buf_size, buff_caps);
                                        ...
                                        uint16_t x_end, uint16_t y_end, uint16_t w, uint16_t h, uint16_t rotation)
{
    ...
    ppa_srm_oper_config_t oper_config = {
        ...
        .scale_x        = 1.0,
        .scale_y        = 1.0,
        .rgb_swap       = 0,  // Panel uses native order
        .byte_swap      = 0,
