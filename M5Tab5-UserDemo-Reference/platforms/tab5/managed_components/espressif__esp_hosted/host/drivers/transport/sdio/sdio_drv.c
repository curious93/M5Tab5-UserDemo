...
void transport_init_internal(void)
{
	...
	// Pre-alloc both SDIO RX slots at boot before any heap fragmentation.
	// Max SDIO frame after 512-byte block-padding = 7168 bytes; we use 8192
	// for headroom. Slots are never freed → SDIO never needs runtime DMA
	// allocation → immune to lwIP-driven DMA heap fragmentation forever.
	for (int _i = 0; _i < 2; _i++) {
		double_buf.buffer[_i].buf = (uint8_t *)MEM_ALLOC(8192);
		if (double_buf.buffer[_i].buf) {
			double_buf.buffer[_i].buf_size = 8192;
			ESP_LOGI(TAG, "SDIO double_buf[%d]: pre-alloc 8192 DMA OK", _i);
		} else {
			ESP_LOGE(TAG, "SDIO double_buf[%d]: pre-alloc FAILED", _i);
		}
	}
